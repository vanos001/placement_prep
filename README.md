...
> namespace identifiers, shell templates) and are correct as written.
> Automated resolution has now been pushed to its limit — structural variants,
> host aliases, Crossref metadata matching and per-site sitemap harvesting were
> each measured and stopped producing trustworthy results. The remainder needs a
> human decision per link (re-point, substitute or drop).
